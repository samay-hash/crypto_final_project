  ...
  const mockApi = {
    ...
    login: async (email, password) => {
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          const foundUser = mockApi.users.find(u => u.email === email && u.password === password);
          if (foundUser) {
    ...
    signup: async (email, password) => {
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          if (mockApi.users.find(u => u.email === email)) {
            reject(new Error('Email already in use'));
          } else {
            const newUser = { id: String(mockApi.users.length + 1), email, password };
        ...
        <main className="pt-20">
          ...
          {(() => {
            switch (page) {
              ...
              case 'dashboard':
                return <ProtectedRoute page="dashboard"><DashboardPage /></ProtectedRoute>;
              case 'watchlist':
                return <ProtectedRoute page="watchlist"><WatchlistPage /></ProtectedRoute>;
              case 'portfolio':
                return <ProtectedRoute page="portfolio"><PortfolioPage /></ProtectedRoute>;
              case 'login':
